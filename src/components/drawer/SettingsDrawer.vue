  ...
  <Drawer :open="show" @update:open="(val) => !val && emit('close')">
    <div class="flex flex-col gap-6 pb-2">
      ...
      <div class="flex flex-col gap-3 max-w-md mx-auto w-full">
        <!-- Theme Selector -->
        <div
          class="backdrop-blur-xl rounded-2xl p-4 bg-[var(--card-bg)] border border-[var(--border)]"
        >
          <ThemeSelector />
        </div>
        ...
        >
          ...
          >
            <div class="flex items-center gap-3">
              <div
                class="w-10 h-10 rounded-full flex items-center justify-center transition-colors bg-[var(--accent-light)]"
              >
                <svg
