...
export function useTimeObjective(
  ...
  const { timeToMinutes, getCurrentWeekDates } = useTimeCalculations(),
    minutesObjective = ref<number>(38 * 60),
    nbDays = ref<number>(5);
  ...
  const remainingMinutes = computed(() => {
    ...
    nbMissingInCurrentWeek = computed(() => {
      const fullMissingDates =
        missingDates.value
          .filter((md) => md.split(" [-] ").length === 1)
          .filter((date) =>
            getCurrentWeekDates().includes(date.split(" [-] ")[0])
          )?.length ?? 0;
      const halfMissingDates =
        (missingDates.value
  ...
  const changeHourObjective = (value: number) => {
    ...
    loadObjective = () => {
      minutesObjective.value =
        Number(loadLocalStorage("hour_objective")) || 38 * 60;
    };
